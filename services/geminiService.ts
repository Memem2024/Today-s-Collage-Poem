...
export const generatePoemImage = async (poemText: string): Promise<string> => {
  ...
  try {
    ...
    // 第二阶段：使用强制性的扁平化安全提示词
    const safePrompt = `A clean, professional minimalist flat illustration of ${keywords}, vector art style, soft and harmonious color palette, aesthetic white space, calm atmosphere, 4k high quality, simplified geometric shapes.`;
